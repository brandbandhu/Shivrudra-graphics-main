...
const resources: ResourceConfig[] = [
  ...
  },
  {
    ...
    icon: PackageCheck,
    columns: ["label", "detail", "product_id", "is_active"],
    fields: [
      { name: "product_id", label: "Product", type: "select", optionsKey: "products" },
      { name: "label", label: "Option Name", required: true, placeholder: "Standard, Premium, Custom Size..." },
      { name: "detail", label: "Option Detail", type: "textarea" },
      { name: "image_url", label: "Card Image", type: "image" },
      {
        name: "colors",
        label: "Card Color Class",
        placeholder: "from-white via-[#f7f7f7] to-[#ffe9e9]",
      },
      { name: "sort_order", label: "Sort Order", type: "number" },
      { name: "is_active", label: "Active", type: "checkbox" },
    ],
...
function AdminResourcePage({ resource }: { resource: ResourceConfig }) {
  ...
  function updateField(field: Field, value: string | boolean) {
    if (field.type === "number" || field.type === "select") {
      setForm((current) => ({ ...current, [field.name]: value === "" ? null : Number(value) }));
      return;
    }
            ...
            <div className="grid gap-4 p-5 md:grid-cols-2">
              {resource.fields.map((field) => (
                <AdminField
                  key={field.name}
                  field={field}
                  options={field.optionsKey ? relationOptions[field.optionsKey] ?? [] : []}
